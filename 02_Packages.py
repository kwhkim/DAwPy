...
# %% [markdown]
# > **R 사용자를 위한 안내:** 파이썬의 **패키지**는 R의 **패키지**와 거의 동일한 역할을 한다. 다른 점은 파이썬이 통계 분석 외에도 여러 가지 다방면의 목적을 위해 사용되는 범용 언어라서 패키지 역시 엄청나게 다양한 목적을 위해 만들어진다. 그리고 한 패키지가 다른 패키지를 사용하기도 하는데 이때 파이썬에서는 패키지 사이의 버전이 중요하다. 왜냐하면 한 패키지 버전이 달라지면 이를 사용하는 다른 패키지가 작동하지 않기도 하기 때문이다. 그리고 파이썬의 버전이 문제가 되어 패키지를 정확하게 실행하지 못하게 되는 경우도 있다. 그렇기 때문에 파이썬에서는 파이썬의 버전과 여러 패키지의 버전이 모두 중요하다. 그런데 R과 달리 파이썬은 보통 하나의 컴퓨터에 하나의 버전만 설치할 수 있다(버전 2와 버전 3을 모두 설치할 수 있지만, 버전 3의 여러 버전을 모두 설치할 수는 없다). 그래서 사람들은 **가상환경**을 구성하여 서로 다른 버전의 파이썬을 하나의 컴퓨터에서 사용할 수 있도록 하였다. 가상환경(virtual environment)란 운영체제의 환경을 가상으로 구성한 것으로, 기존의 환경과 구별되며 서로 분리된 가상 환경을 여럿 만들 수 있다. 가상 환경을 사용하면 각 가상 환경마다 별도의 파이썬, 패키지 등을 설치할 수 있다. 
#
# > R 사용자는 대부분 최신의 R, 그리고 최신의 패키지를 사용하지만, 파이썬 사용자는 필요에 따라 구 버전의 파이썬을 사용하기도 하고, 구 버전의 패키지를 사용하기도 한다. 이런 차이의 가장 큰 원인은 무엇일까? 저자의 생각은 이렇다. R의 공식 패키지인 CRAN의 패키지는 매우 엄격하게 관리되어 R의 버전이 업그레이드될 때마다 패키지도 오류없이 작동되도록 관리자가 유지 보수해야 한다. 하지만 파이썬의 패키지 저장소인 PyPI는 누구나 패키지를 올릴 수 있고, 관리도 전혀 되지 않는 듯 하다. PyPI의 여러 패키지를 확인해보면 10년 동안 아무런 수정이 없는 패키지, 파이썬 버전 2에서만 작동하는 패키지도 수두룩하다. 이때 어떤 목적을 위해 어떤 패키지를 꼭 써야 한다면 어떻게 해야 할까? 패키지가 작동하는 파이썬 버전을 사용할 수 밖에 없는 것이다.
#
# 다양한 버전의 파이썬, 패키지를 한 컴퓨터에서 사용 가능하게 하기 위해 **가상환경**을 사용한다. 
#
...
# %% [markdown]
# 보통 확장자 `.py` 파일에는 파이썬 문(statement)과 함수 등이 들어간다. 특정한 목적을 위해 파이썬 언어로 작성된 `.py` 파일을 모듈(Module)이라고 한다(좀더 정확히는 순수모듈이라고 한다. 자세한 설명은 아래 용어 설명 참조). 그리고 비슷한 목적을 위해 만들어진 여러 모듈을 모아 패키지를 만들기도 한다.
...
#
# * 배포 패키지와 임포트 패키지 : 둘다 보통 **패키지**라고 불린다. 파이썬 공식 사이트의 용어 구분에 따르면, 배포 패키지란 배포를 목적으로 여러 파일을 압축한 **파일**이다. 임포트 패키지란 파이썬에 `import`로 불러들일 수 있는 패키지다. 보통 패키지는 여러 모듈로 구성된다. 
#
# #### **모듈**, **패키지**, **라이브러리(Library)**
#
# **모듈**이란 코드 재사용의 최소 단위로 순수 모듈의 경우 `.py`로 끝나는 파일 하나를 생각하면 된다. 모듈의 길이가 길어지면 필요에 따라 모듈 파일을 두 개 이상으로 나누기도 하고, 복잡한 기능을 조직화하기 위해 하나의 디렉토리에 여러 모듈을 넣거나, 하나의 디렉토리에 하위 디렉토리들을 만들어서 모듈을 나눠 저장하기도 한다. 이렇게 여러 모듈을 모아놓은 것을 **패키지**라고 한다.[^module] 보통 하나의 패키지는 하나의 주제로 통합할 수 있는 여러 기능들이 여러 모듈에 나눠 담겨 있다. **라이브러리**는 여러 패키지들을 모아 부르는 말이다. 예를 들어 파이썬 표준 라이브러리([The Python Standard Library](https://docs.python.org/3.8/library/index.html))에는 파이썬과 함께 배포되는 자주 쓰이는 패키지 또는 모듈을 담고 있다. 파이썬 표준 라이브러리에 속하지 않는 패키지를 보통 제3자 패키지(third-party package)라고 부른다. 
#
# * 내장 모듈 
#   - 내장(built-in) 모듈은 다른 모듈과 달리 파일로 존재하지 않고, Python 안에 존재하는 C로 쓰여진 모듈이다.
#   - 내장 모듈을 불러들이기 위해서는 다른 모듈과 마찬가지로 `import` 문을 쓴다.
#   - 하지만 그렇게 불러들인 모듈에 `.__file__`은 존재하지 않는다(예. `import sys; sys.__file__`). 보통 모듈의 `.__file__`은 모듈이 저장되어 있는 파일의 경로와 이름이 담겨 있다.
#   - 내장 모듈 리스트는 `sys.builtin_module_names`로 확인할 수 있다. 
#   
# [^module]: 하지만 파이썬 내에서는 이런 구분 없이 import의 결과는 모두 module이라고 표시하는 듯 하다. `import`한 결과를 출력해보면 `<module '...' from 'C:\\Users\\Home\\...\\__init__.py'>`와 같이 모두 `module`로 표시된다. 만약 패키지로 디렉토리를 구성한다면 파일 이름(`.__file__`)은 모두 `__init__.py`가 된다.
#
#
...
# %% [markdown]
# `string`은 파이썬 표준 라이브러리의 모듈로 `string.py`에 저장되어 있다. 
...
# %% [markdown]
# `matplotlib`은 제3자 패키지의 하나이다. `matplotlib`은 `.../matplotlib` 폴더 안의 여러 폴더 및 모듈로 구성된다. 
...
# %% [markdown]
# `matplotlib` 패키지에는 `pyplot`이라는 모듈이 있다.[^importmatplotlib]
...
# %% [markdown]
# [^importmatplotlib]: 패키지 `matplotlib` 안의 모듈 `pyplot`을 불러오는 두 가지 방법을 보여주었다. 바로 설명하겠지만 `from matplotlib import pyplot` 또는 `import matplotlib.pyplot`이 가능하다. 임포트하면서 이름을 변경할 수 있는데 `import matplotlib.pyplot as pyplot`을 한다면 실질적으로 `from matplotlib import pyplot`과 동일한 결과를 얻는다.
...
#   - 일부 내장 모듈을 포함한다.  
#   - 내장 모듈 빌트인즈(`builtins`)에는 내장 함수(built-in function)들이 정의되어 있다. `__builtin__`, `__builtins__`은 모두 내장 모듈 `builtins`를 가리킨다. (보통 `__`로 시작하고 `__`로 끝나는 이름은 내부용이다. 내부용은 후에 어떻게 바뀔지 장담할 수 없다. 비록 지금 `__builtin__`과 `builtins`가 동일하다고 해서 파이썬의 후속 버전에서도 그러리라는 보장이 없으므로 `__builtin__` 대신 `builtins`를 쓰는 것이 좋다. 내장 모듈의 함수는 `builtins.print`로 쓸 필요없이 그냥 `print`로 쓸 수 있다.
...
# `print(json.__doc__)`와 같이 `.__doc__` 속성으로 패키지 설명도 확인할 수 있다. `import`된 모듈은 `sys.modules`에 사전 형식으로 기록된다.
#
# #### `import` 사용법
#
