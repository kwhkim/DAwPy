...
# %% [markdown]
# `arr2`는 `arr1`의 별칭(alias)이고, `arr6`는 `arr1`의 뷰(view)이다. 별칭이란 완전히 동일한 객체이고 이름만 다르다는 의미이다. 넘파이 뷰는 동일한 객체를 다른 방식으로 일부 또는 전체의 데이터를 해석한다는 의미이다. 예를 들어 이진수 `0b0010001100000001`를 예로 들어보자. 이를 8비트 정수형으로 인식하면 `35`와 `1`이 된다. 하지만 16비트 정수형으로 해석하면 `8961`로 전혀 다르게 해석된다(`0b00100011`, `0b00000000`, `0b0010001100000000`을 콘솔에 입력해보자). 만약 이를 8비트씩 끊어서 `0`은 `False`, `0`이 아닌 경우를 `True`를 해석하면, `0b0010001100000000`는 `True`, `False`로 해석된다. 

# %%
import numpy as np
a1 = np.array([0b0010001100000001], dtype='int16')
a1

# %%
a2 = a1.view(dtype='int8')
a2

# %%
a3 = a1.view(dtype='bool')
a3

# %% [markdown]
# 그래서 `a2`와 `a3`는 `a1`의 뷰라고 한다. 뷰는 이렇게 메모리 상의 정보를 다른 dtype으로 해석할 수도 있고, 메모리 상의 정보중 일부만을 선택할 수도 있다. 다음을 보자.
...
# %% [markdown]
# `b`는 `a`와 동일한 dtype이지만 `a`의 정보 중 일부만을 사용한다. 이 경우도 뷰라고 한다.
