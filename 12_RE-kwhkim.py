...
# %% [markdown]
# 가장 먼저 알아둘 것은 `(`,`)`는 안에 정규표현식이 있다면, 정규표현식을 먼저 해석한 후 `(`, `)` 밖의 정규표현식을 해석하게 된다는 점이다. 예를 들어 정규표현식 `a.*(a|b)`는 `a`로 시작하고 `a` 또는 `b`로 끝나는 패턴을 찾는다. 만약 (`a.*a|b`로 쓴다면 `a`로 시작하고 `a`로 끝나는 패턴)이나 `b`를 찾게 된다.
