...
# `[`(a, i)는 `[`라는 함수에 인자로 a와 i를 넣은 것으로,
# 보통 a라는 변수(객체)의 i-번째 원소를 의미한다.

# Python에서 class A의 instance a에 a[i]를 하면,
# A의 special method __getitem__(self, position)이 실행된다.
# 역시 보통 a의 i-번째 원소를 의미한다.

# ## R의 :과 파이썬의 :

# R에서 :은 seq() 함수를 간편하게 표기하는 방법이다.
# R에서 1:3은 c(1,2,3)이고 3:1은 c(3,2,1)이다.

# 파이썬에 :은 slice() 함수를 간편하게 표기하는 방법이며,
# [과 ] 안에서만 사용할 수 있다.
# 파이썬에서 a[0:3]은 a[0], a[1], a[2]을 의미하고,
# a[3:0]은 None이다. a[2::-1]을 해야 a[2], a[1], a[0]이 된다.
