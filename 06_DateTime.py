...
# %% [markdown]
# 우리가 `datetime.datetime.now()`로 얻은 날짜시간은 시간대에 대한 정보가 없다. 이렇게 시간대 정보가 없는 날짜시간형을 naive라고 하고 시간대 정보가 있는 날짜시간형을 aware라고 한다. naive 날짜시간형은 본인의 컴퓨터가 다른 시간대로 옮겨간다던지 다른 시간대의 컴퓨터로 정보를 옮기면 그 정확한 의미를 확인할 수 없기 때문에 시간대 정보(timezone/offset)를 추가해주는 것이 좋다.
