...
# ### 최상의 전략과 이유?
#
# 1. 과거 시간은 UTC를 사용한다 : UTC와 시간대를 알면 해당 시간대로 시간을 구할 수 있다.
# 2. 미래 시간은 날짜시간과 시간대를 함께 저장한다 : 미래에 어떻게 시간대 또는 써머타임이 변할 지 알 수 없다. 특정 시간대로 기술된 미래 시간은 UTC로 확정할 수 없다. 왜냐하면 시간대의 offset 또는 dst가 어떻게 변할지 미리 알 수 없기 때문이다.
