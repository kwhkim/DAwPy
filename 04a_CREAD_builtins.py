...
# %% [markdown]
# `list`, `tuple`, `dict`, `set`는 여러 객체를 담는 용도로 쓰이기 때문에 container라고 불린다. 이번 장에서는 파이썬의 내장 container를 활용하는 방법을 다룬다. 이들은 데이터를 저장하기 위해 사용할 수도 있지만 `for`와 같은 반복문을 활용하기 위해서도 많이 활용된다.

# %% [markdown]
# > **R과 비교** : R에서 주로 사용하는 데이터 구조는 벡터이다. 반면 파이썬에서는 리스트를 광범위하게 사용한다. 파이썬의 리스트는 R의 리스트와 마찬가지로 어떤 종류의 객체도 원소로 저장할 수 있다는 점에서 비슷하지만, R의 리스트가 원소를 모두 저장한다고 생각해도 무방한 반면 파이썬의 리스트는 어떤 객체를 가리키는 **포인터**의 모음이라는 점에서 차이가 있다. 그리고 이런 차이가 R 사용자가 파이썬에서 헤매는 원인을 제공한다. 따라서 R의 벡터 또는 리스트와 파이썬의 리스트의 차이점을 반드시 숙지할 필요가 있다. 

# %% [markdown]
# ### 먼저 객체

# %% [markdown]
# 앞에서 파이썬의 거의 모든 것은 **객체**(object)라고 했다. 예를 들어 정수 `3`도, 실수 `3.14`도 객체이다. 함수도 객체이고, 모듈도 객체이고, 아무튼 뭐라고 부를 수 있는 것은 객체라고 생각해도 좋다. 구체적으로 어떤 변수에 할당할 수 있는 무엇은 **객체**이다.
...
# %% [markdown]
# 이런 방식에서 변수는 어떤 값을 저장하지 않는다. 단지 메모리에 존재하는 **객체를 가리킬 뿐이다.** 메모리의 한 주소를 가리킨다는 점에서 컴퓨터 공학에서는 흔히 포인터(pointer)라는 용어를 사용하기도 한다. (하지만 포인터는 값은 주소이지만, 파이썬 **변수의 값**은 해당 객체이다.) 객체 레퍼런스(object reference)라고 말하기도 한다. 모두 객체 자체를 저장하는 것이 아니라 객체를 가리킨다고 이해할 수 있다. 
...
# %% [markdown]
# 리스트는 `[`,`]`안에 원소를 나열하여 생성한다. `[3,2,5]`를 하면 원소가 `3`,`2`,`5`인 리스트가 만들어지고 변수 `lst`는 이 객체(리스트)를 가리키게 된다. 이 리스트의 첫 번째 원소는 `lst[0]`, 두 번째 원소는 `lst[1]`으로 표기한다. 그렇다. 파이썬은 0번째부터 시작한다. 그래서 `lst[0]`을 0번째 원소라고도 말한다. 헷갈리지 않는가? 당연히 `lst[0]`을 첫 번째 원소라고 부르다가 0-번째 원소라고 부르다가 다시 1-번째 원소라고 부르면 헷릴 것이다. 이 책에서는 파이썬을 얘기할 때에는 항상 0-번째부터 시작한다.
...
# %% [markdown]
# 정리를 해보자. 불변객체 `3`은 바뀌지 않는다. 가변 객체 리스트는 변할 수 있다. 그런데 리스트가 무엇인가? 리스트란 단지 일정한 갯수의 포인트이다. (혹은 `x=3`에서 `x`와 같은 변수가 여럿 있는 것으로 생각할 수도 있다.)

# %% [markdown]
# 다시 한번 `lst=[1,3,2]`가 내부적으로 어떻게 작동하느지 도식적으로 상상해보자. `[1,3,2]`를 하면 포인터 3개짜리 리스트가 만들어진다. (그리고 **가변 객체** 리스트는 이 3개의 포인트가 가리키는 대상이 변할 수 있다.) 그리고 불변 객체 `1`,`3`,`2`는 메모리 상의 어딘가에 생성된다. (그리고 결코 변하지 않을 것이다.) 리스트의 세 포인트는 차례로 `1`, `3`, `2`를 가리킨다. 그리고 변수 `lst`는 (포인트가 차례로 `1`,`3`,`2`를 가리키는) 방금 생성한 리스트를 가리킨다.
...
# %% [markdown]
# `id()` 결과를 보면 알 수 있듯이 `lst1`, `lst2`, `lst3`는 서로 다른 객체이다. 
# 그런데 `id()`로 나오는 값은 크게 중요하지 않다. 중요한 것은 두 변수의 `id()` 값이 같냐 틀리냐이다. 
# 이를 쉽게 알 수 있는 방법은 `is` 연산자를 사용하는 것이다. 만약 두 객체 `a`, `b`가 동일한 경우 `a is b`는 `True`가 나오고, 그렇지 않으면 `False`이다.
...
# %% [markdown]
# 다음은 가변객체인 딕에 대해서도 위에서 설명한 상황이 펼쳐진다는 점을 분명히 보여준다. (딕은 바로 다음에 설명한다.) `.copy()`를 해도 `lst3[1]['a'][2]`의 변화는 `lst2`, `lst1`의 출력도 변화시킨다. 하지만 `copy.deepcopy()`를 사용할 경우에는 `lst1`, `lst2`, `lst3`는 서로 분리된다.
...
# %% [markdown]
# 반면 변수 `x`가 가리키는 것이 **가변 객체**이고, 다른 변수 `y`도 동일한 **가변 객체**를 가리킨다면, 변수 `y`를 통해 수정이 가능하다. 이런 경우에는 변수 `x`를 사용하지 않았지만 `x`의 내용이 달라진다.
...
# %% [markdown]
# 이 장에서는 파이썬의 가장 기본적인 데이터 구조인 `list`, `tuple`, `dict`, `set`에 대한 데이터 처리 방법을 배운다. `list`와 `tuple`은 가변, 불변 쌍둥이라고 할만큼 가변성을 제외하고는 비슷하다. `dict`, `set`은 모두 가변객체이다. `set`은 가변객체이지만, 그 원소가 가변객체가 될 수는 없다.
...
# 딕는 **키**(key)를 사용한다.[^dictkey] 예를 들어, 키가 Mary인 원소는 뭐냐? 키가 5인 원소는 뭐냐?라는 질문에 빠르게 대답할 수 있다.
# 셋은 우리가 흔히 말하는 **집합**이다. 집합은 순서가 없고, 있냐 없느냐(존재 유무)만 구분 가능하다. 이를 **원소 자신**이 키인 경우로 생각해 볼 수 있다. 그래서 `"Mary"`가 있느냐? `5`가 있느냐?라는 질문에 빠르게 답할 수 있다.
#
...
#
# [^dictkey]: 구체적인 구현 방법을 제외하면 R 벡터 **이름** 또는 데이터 프레임의 행/열**이름**과 같은 개념이다. R에서 이름은 문자열만 가능하지만 파이썬의 키(key)는 문자열, 정수형, 실수형이 모두 가능하다는 차이가 있다.
...
# 이에 대해서는 나중에 다룬다.
# 원소를 확인하고 싶다면 iterator에 list()를 한다.

# %%
reversed(elems_add)

# %%
list(reversed(elems_add))
...
# %%
# 함수, Generator, Iterator에 대해 알고 있어야
def reversed2(x):
    i = len(x)-1
    while i > -1:
        yield x[i]
...
# %% [markdown]
# * `list()` : 파이썬 `list()`는 튜플, 셋, 딕을 리스트로 바꾸는 역할을 하지만 새로운 리스트를 생성하는 역할은 없다. R의 경우 `list(1,3,2)`를 하면 원소 `1`,`3`,`2`의 리스트가 만들어진다. 파이썬에서도 이런 식으로 사용하려면 다음과 같이 한다.[^pythonlist]
#
...
# %% [markdown]
# `x[len(x)]`와 같이 존재하지 않는 순번을 요구하면 IndexError가 발생하게 된다. 하지만 신기하게 `x[len(x):len(x)-1:-1]`은 IndexError가 발생하지 않는다. 다음의 코드와 유사하게 이해하자
#
# ```{python}
# res = []
# i = len(x)
...
# %% [markdown]
# 그래서 `x[i:i+1]`은 `x[i]`와 동일하지만 `IndexError`가 발생하지 않는다.
...
# %% [markdown]
# `del y`는 `y`가 가리키는 튜플을 삭제하는 것은 아니다. 다음을 보자.
...
# %% [markdown]
# 이렇게 다른 변수가 변수 `x`가 가리키는 객체를 가리킨다면 객체까지 삭제할 수 없다. 파이썬은 객체를 가리키는 변수가 남아있는지를 확인하여 객체를 가리킬 수 있는 방법이 더 이상 없을 때 메모리 관리 차원에서 객체를 삭제한다.
