...
# ### `.to_csv()`와 `to_pickle()`
#
# 데이터프레임을 텍스트 파일로 저장하는 가장 대표적인 방법은 `.to_csv()` 메쏘드를 활용하는 것이다. 파일 이름과 인코딩 방법을 적어준다. 인코딩 방법을 생략하면 운영체제의 기본 인코딩 방법을 사용한다. 요즘은 UTF-8 인코딩을 사용하는 것이 좋다(`encoding = 'UTF-8'`). 뒤에서 설명하겠지만 UTF-8은 전세계적으로 가장 많이 쓰이는 인코딩 방법이다(윈도우에서 UTF-8로 설정하지 않으면 윈도우의 기본 인코딩인 `CP949`로 설정된다).
#
...
# %%
with open('data/movies.pkl', 'rb') as f1:
    with open('data/movies2.pkl', 'rb') as f2:
        s1 = f1.read()
        s2 = f2.read()
        for x1, x2 in zip(s1, s2):
            if x1 == x2:
                ...
                print(f'{x1:02x}:{x2:02x}', end=' ')
# 아래의 출력은 `movies.pkl`의 파일 내용과 `movies2.pkl`의 파일 내용이 어떻게 다른지 보여준다.
# :의 왼쪽은 movies.pkl, :의 오른쪽은 movies2.pkl 내용이다. 
# 두 파일의 내용이 같다면 :의 왼쪽만 사용한다.
