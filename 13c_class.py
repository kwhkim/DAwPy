...
# %% [markdown]
# 클래스 `Patient` 아래 `upname()`이라는 함수를 정의하였다. `upname()`의 정의를 살펴보면 `self`라는 인자를 받는다. 그런데 `w.upname()`을 보면 인자가 주어지지 않는 것처럼 보인다. 이것이 함수(function)과 메소드(method)의 차이이다. Patient3라는 클래스에 포함된 upname이라는 함수는 upname(self) 처럼 인자가 필요하다. 그래서 Patient3.upname(x)과 같이 사용해야 한다. 그런데 `Patient`에서 정의된 `upname`은 객체를 통해 접근할 수도 있다. 이렇게 `w.upname`으로 쓰면 동일한 함수에 접근하게 되는데 객체를 통해 접근했기 때문에 첫 인자는 자동으로 객체 `w`가 입력되는 특별한 함수가 되는 것이다. 이런 함수를 특별히 메소드라고 부른다. 
# 
# 메소드는 객체의 속성으로 함수를 호출한다. 객체의 속성으로 함수를 호출한다는 것은, 가령 위에서 볼 수 있듯, w라는 객체에 Patient3('han seok-bong') 이라는 값을 부여한 후, `w.upname()`처럼 함수를 호출하는 방식이라고 볼 수 있다.  그리고 첫 번째 인자는 항상 현재 객체(`w.upname()`에서 `w`)가 입력된다. 
...
# %% [markdown]
# 인스턴스 메소드는 클래스의 속성으로 존재하는 함수를 인스턴스를 통해 접근하여 첫 번째 인자가 자동으로 인스턴스가 되는 것이다. 클래스 메소드는 첫 번째 인자로 클래스가 된다. 정적 메소드는 자동으로 입력되는 첫 번째 인자가 없다. 세 메소드의 공통점은 클래스 아래에서 생성된다는 것이다. 
# 인스턴스 메소드가 가장 기본적이다. 클래스 아래 존재하는 함수를 객체를 통해 접근하면 인스턴스 메소드가 되어 첫 번째 인자는 인스턴스가 된다.
# 클래스 메소드를 정의할 때에는 `@classmethod`라는 **데코레이터**[^deco]가 붙으며, 이때에는 클래스 또는 객체를 통해 함수를 접근할 때 첫 번째 인자로 클래스가 입력된다.
# 정적 메소드를 정의할 때에는 `@staticmethod`라는 데코레이터가 붙으며, 이 함수는 클래스 또는 객체를 통해 접근해도 일반 함수처럼 작동한다. 
# [^deco]: 함수 정의 직전에 `@`가 붙은 부분을 데코레이터라고 한다. 
...
# %% [markdown]
# 정적 메소드와 클래스 메소드, 인스턴스 메소드의 차이는 첫 인자로 자동으로 입력되는 값이 무엇이냐이다. 정적 메소드는 일반적인 함수처럼 첫 인자로 자동으로 입력되는 값이 없다. 클래스 메소드는 클래스가 첫 인자가 되고, 인스턴스 메소드는 첫 인자로 인스턴스가 입력된다. 세 메소드는 모두 클래스를 통해 접근할 수도 있고, 인스턴스를 통해 접근할 수도 있다. 
# 정적메소드(staticmethod)와 인스턴스 메소드(instance method), 클래스 메소드(classmethod)의 공통점은 모두 함수 정의가 클래스 아래 존재한다는 것이다. 이들은 모두 클래스의 속성으로 취급하기 때문에 클래스에 점(`.`)을 붙여 접근 가능하다. 이때 자동으로 첫 번째 인자가 설정되는 것은 클래스 메소드 뿐이다. 클래스 메소드를 클래스를 통해 접근하면 첫 번째 인자는 자동으로 접근에 사용한 클래스가 된다.  
# 세 메소드는 객체를 통해 접근할 때 차이가 두드러진다. 정적 메소드를 객체를 통해 접근해도 그냥 함수로 작동한다. 자동으로 첫 번째 인자가 설정되지 않는다. 인스턴스 메소드를 객체를 통해 접근하면 첫 번째 인자로 접근에 사용된 객체가 설정된다. 클래스 메소드를 객체를 통해 접근하면 첫 번째 인자로 접근에 사용된 객체의 클래스가 사용된다. 클래스 메소드를 왜 객체를 통해 접근하냐고 물어볼 수도 있겠지만 클래스 이름을 모를 수도 있는 것 아닌가? (`x`라는 객체라면 `x.__class__.<method>`를 간단하게 쓴 것이라고 생각할 수도 있다. 하지만 `x.<method>`는 `x`에 `<method>`라는 속성이 존재할 수도 있기 때문에 항상 동일하게 작동할 것이라고 생각할 수는 없다. 궁금하다면 `print(x.<method>)`와 `print(x.__class__.<method>)`로 둘을 비교해보는 것도 좋다.)
# 
# 이들은 어떻게 사용되는 것이다. 클래스는 객체들을 모두 모은 것이라고 생각할 수 있다. 클래스에 속하는 인스턴스 전체에 대한 사항은 클래스 속성 또는 클래스 메소드를 사용해야 할 것이다. 개별 인스턴스에 대한 기능은 인스턴스 메소드로 작성해야 개별 인스턴스에 대한 조작이 가능하다. 정적 메소드는 굳이 메소드로 만들 필요가 없지만(함수로 만들어도 동일하게 사용할 수 있지만), 논리적으로 클래스와 긴밀한 관계가 있는 함수라면 클래스 아래 정의하는 것이 프로그래밍에 편하다고 생각할 때 사용할 수 있다. 
