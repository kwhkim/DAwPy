...
# %% [markdown]
# 클래스 `Patient` 아래 `upname()`이라는 함수를 정의하였다. `upname()`의 정의를 살펴보면 `self`라는 인자를 받는다. 그런데 `w.upname()`을 보면 인자가 주어지지 않는 것처럼 보인다. 이것이 함수(function)과 메소드(method)의 차이이다. Patient3라는 클래스에 포함된 upname이라는 함수는 upname(self) 처럼 인자가 필요하다. 그래서 Patient3.upname(x) 이런 식으로 명령어를 입력해야 한다. 반면, 메소드는 객체의 속성으로 함수를 호출한다. 객체의 속성으로 함수를 호출한다는 것은, 가령 위에서 볼 수 있듯, w라는 객체에 Patient3('han seok-bong') 이라는 값을 부여한 후, `w.upname()`처럼 함수를 호출하는 방식이라고 볼 수 있다.  그리고 첫 번째 인자는 항상 현재 객체(`w.upname()`에서 `w`)가 입력된다. 
...
# %% [markdown]
# 인스턴스 메소드는 클래스의 속성으로 존재하는 함수를 인스턴스를 통해 접근하여 첫 번째 인자가 자동으로 인스턴스가 되는 것이다. 클래스 메소드는 첫 번째 인자로 클래스가 된다. 세 개의 공통점은 정의가 클래스에 있다는 것. 클래스의 이름공간에 함수가 정의되어 존재한다.
...
# %% [markdown]
# 정적 메소드와 클래스 메소드, 인스턴스 메소드의 차이는 첫 인자로 자동으로 입력되는 값이다. 정적 메소드는 일반적인 함수처럼 첫 인자로 자동으로 입력되는 값이 없다. 클래스 메소드는 클래스가 첫 인자가 되고, 인스턴스 메소드는 첫 인자로 인스턴스가 입력된다. 세 메소드는 모두 클래스를 통해 접근할 수도 있고, 인스턴스를 통해 접근할 수도 있다. #staticmethod와 인스턴스 메소드, 클래스 메소드의 차이는 일단 정의가 클래스의 이름공간에 있는거고 그걸 객체를 통해서 그 이름공간의 함수에 접근하여 사용할 때, 첫 번째 인자가 뭐가 들어가느냐의 차이. classmethod는 객체의 class가 첫 번째 인자, 인스턴스메소드는 객체의 인스턴스가 첫 번째 인자로 들어가는 것, 정적 메소드는 인자가 아무것도 들어가지 않아서 일반 함수와 동일하게 사용 가능.
#
# 클래스가 있고 인스턴스가 있을 때 인스턴스는 각각 개별적으로 작동을 하는 거고. 정적 메소드는 꼭 거기 있을 필요는 없지만 이 클래스와 긴밀하게 쓸 수 있는 함수다. 인스턴스 메소드는 인스턴스를 객체를 통해 접근하는 게 자연스럽고, 클래스 메소드는 클래스라는 객체를 통해 접근하는 게 자연스럽다. 
