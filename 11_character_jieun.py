...
# +++ 유니코드에 대한 기본적인 내용
#
# 유니코드의 모든 문자를 지원하는 폰트가 없다. 폰트란 글꼴(굴림체, 고딕체..). 폰트를 지원하지 않는 문자(다른 나라 문자)는 화면에 제대로 출력되지 않는다. 같은 문자도 다른 식으로 유니코드로 표현할 수 있으니까. '가'를 'ㄱ+ㅏ'이렇게 저장할 수도 있는데 똑같이 '가'로 출력되니까 구분이 안 될 수도 있는데, 동일해 보이거나 출력이 안 되는 문자의 유니코드를 확인하는 방법이라고 생각할 수 있다. 
