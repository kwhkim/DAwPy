...
#
# [^forstring]: 하지만 `.join()`은 좀더 효율적인 방식으로 구현된다. `for` 안의 `res = res + s + x`를 보면 `res`가 가리키는 객체는 조금씩 커져간다. 이렇게 조금씩 커져가는 객체를 생성하는 것보다 모든 구성요소를 모아서 한꺼번에 하나의 객체를 만드는 것이 훨씬 효율적이다. 데이터 프레임을 합칠 때에도 비슷하다.
