...
# +++ 유니코드에 대한 기본적인 내용
#
# 하나의 폰트(글꼴, 예. 굴림체, 고딕체)이 지원하는 글자의 갯수는 글꼴 타입에 따라 다르지만 한정적이기 때문에[^supportfont] 대부분의 글꼴은 유니코드에서 제공하는 모든 문자를 화면에 표시할 수 없다. 하나의 글꼴이 지원하지 못하는 글자를 위해 보조 글꼴을 사용하기도 하지만 이 또한 유니코드의 모든 문자를 지원하기에는 턱없이 부족하다. 따라서 유니코드 문자열을 `print()` 등의 함수로 출력할 때에 모든 글자가 정확하게 표시되리라 기대할 수 없다.
# 그리고 유니코드는 동일한 문자를 여러 가지 방법으로 저장할 수 있기 때문에 같은 문자가 표시되더라도 동일한 문자열이라고 확신할 수도 없다[^unicodedecompose].
# 다음의 함수 `unicode_info()`는 현재 글꼴이 지원하지 않는 문자에 대해서도 유니코드 코드 포인트와 유니코드 레이블을 확인할 수 있고, 화면에 동일하게 표시되는 문자에 대해서도 유니코드 코드포인트 상의 차이를 확인할 수 있다. 
# 
# [^supportfont]: https://en.wikipedia.org/wiki/Unicode_font
# [^unicodedecompose]: 예를 들어 한글 `가`는 한 글자 `가`로 저장될 수도 있고, `ㄱㅏ`와 같이 초성 기억과 중성 ㅏ로 저장될 수도 있다. 
